...
### Phase 13: LPG target layout -- implemented (P13.1–P13.10)
...
| **P13.10 ✅** | **Multi-label semantics and their filter contract.** `labels` on a node and `fromLabels`/`toLabels` on an edge are **arrays**, and must stay correct — and index-served — when they hold several labels. Label tests use AQL array-comparison operators, which map 1:1 onto Cypher's label expressions: `@l IN arr` = `:A`; `@ls ALL IN arr` = `:A:B`; `@ls ANY IN arr` = `:A\|B`; `@ls NONE IN arr` = negation. r2g exposes these through `label_predicate()` so a consumer does not have to derive them. **A label filter MUST be written against the edge variable inside the traversal, never against `p.edges[*]`:** for a *list-valued* field that accessor expands to a **nested** array, so both `… ALL == @l` and `@l IN p.edges[*]…` match nothing and return an **empty result with no error** — the inverse of the scalar-field trap, where `[*] ALL ==` over a single edge is trivially *true*. `FLATTEN()` works but expresses only "some hop", because flattening discards which hop each element came from; chained array operators (`ALL ANY ==`) are a syntax error. The edge-variable form is applied per hop by construction and is the only form the vertex-centric index can serve. | `src/r2g/lpg.py:117` (`LABEL_MODES`), `:120` (`label_predicate`, which refuses to emit a `[*]` accessor). Verified live on ArangoDB 3.12.9 with 3-label nodes and multi-element edge copies: all four operators correct **and** engaging the VCI; a test pins the two broken `[*]` forms returning `[]` so the trap cannot regress quietly. |
