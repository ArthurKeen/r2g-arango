...
| **P13.3 ✅** | **Vertex-centric indexing.** The edge indexes are `[_from, type]` and `[_to, type]`, and each edge additionally carries copies of its endpoints' labels (`fromLabels`/`toLabels`) as **non-indexed** attributes. The copies are load-bearing but not as index fields: they let a label filter be satisfied from the edge document the traversal has already loaded, without materializing the neighbour vertex — were the labels left only on the neighbour, every candidate would have to be fetched to be rejected. **The label arrays must not be indexed on the edge collection.** A persistent index over an array-expansion field stores *one entry per element*, and a traversal scans every entry under the `_from`/`type` prefix, so indexing `toLabels[*]` returns each edge **once per label on its target** — the multiplier being introduced by exactly the multi-label support the arrays exist for, and the symptom being inflated counts rather than an error. There is no query-side repair: the label filter does not narrow the prefix scan, `uniqueEdges: 'global'` is rejected (ERR 10), and `RETURN DISTINCT` merely hides the symptom while the traversal still does the work and any edge aggregate stays inflated. Nothing is lost by dropping the field: the flat spec is still selected and still serves the pushed-down `p.edges[*].type ALL == @type` condition. The expansion remains correct on the **node** collection, where an equality lookup matches one entry per document and the access path is an `IndexNode` rather than a traversal, so `labels[*]` continues to serve traversal entry points. | `src/r2g/lpg.py:92` (`edge_indexes`), `:39` (`vertex_indexes`), `:68` (the rationale). Measured on 3.12.11 with targets carrying 1/2/3/4 labels: the array spec returned them 1/2/3/4 times; the built-in edge index and the flat spec returned each exactly once. Regression-guarded by `TestLpgNoDuplicationFromLabelArrays`, which counts occurrences (never `DISTINCT`s them) and structurally asserts no edge index carries a `[*]` field. |
