...
### Phase 13: LPG target layout -- implemented (P13.1–P13.11); P13.12 guarded, full support pending
...
- **r2g emits exactly one label per node** (and one per edge endpoint): a table
  maps to a single label, and `CollectionMapping` carries no label list. So
  multi-label is *queryable and indexed* — see P13.10 — but **not producible**
  by r2g. Multi-label documents in an r2g-loaded graph can only come from
  elsewhere.
